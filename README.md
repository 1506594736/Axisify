...
3. 在「**实体化**」分组里设**厚度**和**方向**（默认会自动给物体添加 Solidify 修改器；已经有的就更新参数）
4. 点「**对齐侧壁到轴**」

结果会生成一个**新物体**（原名 + `_对齐`），原物体不动。

> 也可以自己先加好 Solidify，再取消勾选「自动设置实体化」，插件会沿用它。
> 点「从修改器读取」可以把物体上现有 Solidify 的厚度 / 方向读回面板。
...
| **厚度** | 0.1 | Thickness | 实体化厚度。侧壁对齐后**墙长就等于它** |
